...
const WALL_STUCK_THRESHOLD := 0.05
# test
const UNSTUCK_STEP : float = 5.0
# test
const MAX_UNSTUCK_DISTANCE : float = 1000.0
# test
const PLAYER_SIZE : float = 20
...
# test
@onready var _unstuck_rays: Node2D = $UnstuckRays
...
func _handle_wall_stuck() -> void:
	var current_distance = UNSTUCK_STEP
	while current_distance <= MAX_UNSTUCK_DISTANCE:
		for ray: RayCast2D in _unstuck_rays.get_children():
			ray.target_position = ray.target_position.normalized() * current_distance
			ray.force_raycast_update()
			
			if not ray.is_colliding():
				global_position += (ray.target_position + ray.target_position.normalized() * PLAYER_SIZE)
				velocity = Vector2.ZERO
				print("Unstuck player using ray: ", ray.name, " at distance: ", current_distance)
				return
		
		current_distance += UNSTUCK_STEP
	
	print("Failed to unstuck player")
